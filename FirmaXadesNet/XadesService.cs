...
namespace FirmaXadesNet
{
    ...
    public class XadesService
    {
        ...
        /// <param name="parameters"></param>
        public SignatureDocument Sign(XmlDocument inputXml, SignatureParameters parameters)
        {
            if (parameters.Signer == null)
            {
                throw new Exception("Es necesario un certificado válido para la firma");
            ...
            if (inputXml == null && string.IsNullOrEmpty(parameters.ExternalContentUri))
            {
                throw new Exception("No se ha especificado ningún contenido a firmar");
            }
            //var _inputXml = inputXml;
            //using (MemoryStream xmlstream = new MemoryStream())
            //{
            //    inputXml.Save(xmlstream);
                ...
                switch (parameters.SignaturePackaging)
                {
                    case SignaturePackaging.INTERNALLY_DETACHED:
                        ...
                        if (!string.IsNullOrEmpty(parameters.ElementIdToSign))
                        {
                            SetContentInternallyDetached(signatureDocument, inputXml, parameters.ElementIdToSign);
                        }
                        else
                        {
                            //SetContentInternallyDetached(signatureDocument, inputXml);
                        }
                        break;
                    ...
                    case SignaturePackaging.HASH_INTERNALLY_DETACHED:
                        ...
                        _dataFormat.Encoding = "http://www.w3.org/2000/09/xmldsig#base64";
                        SetContentInternallyDetachedHashed(signatureDocument, inputXml);
                        break;

                    case SignaturePackaging.ENVELOPED:
                        _dataFormat.MimeType = "text/xml";
                        _dataFormat.Encoding = "UTF-8";
                        SetContentEnveloped(signatureDocument, inputXml);
                        break;
        ...
        /// <param name="content"></param>
        /// <param name="mimeType"></param>
        private void SetContentInternallyDetachedHashed(SignatureDocument sigDocument, XmlDocument inputXml)
        {
            sigDocument.Document = new XmlDocument();
            ...
            using (SHA256 sha2 = SHA256.Create())
            {
                ...
                inputXml.Save(xmlstream);
                contentElement.InnerText = Convert.ToBase64String(sha2.ComputeHash(xmlstream));
            }
